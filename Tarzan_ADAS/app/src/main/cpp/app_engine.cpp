...
AppEngine::RenderFrame ()
{
    ...
    static double ttime[10] = {0}, interval, depth_invoke_ms, deeplab_invoke_ms, detect_invoke_ms;
    ...
    int count = glctx.frame_count;
    {
        ...
        sprintf (strbuf, "Interval:%5.1f [ms]\nDepth  :%5.1f [ms]\nDepth  :%5.1f [ms]\nDetect  :%5.1f [ms]", interval, depth_invoke_ms, deeplab_invoke_ms, detect_invoke_ms);
...
AppEngine::InitGLES (void)
{
    ...
    asset_read_file (m_app->activity->assetManager,
                     (char *)DEEPLAB_MODEL_PATH, m_tflite_detect_model_buf);

    ret_deeplab = init_tflite_dense_depth (
